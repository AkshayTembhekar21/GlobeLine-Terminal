...
public class CoreAggregatorService {
	...
	/**
	 * Sets the event observer (used by decorator pattern).
	 * Called after construction to wire up the observer pattern.
	 */
	 ...
	 * 
	 * This method contains ONLY business logic - no metrics, no cross-cutting concerns.
	 * 
	...
	public Mono<TickerOverviewDto> getOverview(String symbol) {
		...
		if (cached != null) {
			...
			// Notify observer about cache hit - observer handles metrics
		...
		if (inFlight != null) {
			...
			// Notify observer about in-flight sharing
			if (eventObserver != null) {
